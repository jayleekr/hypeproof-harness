...
확장하기 위한 구조를 정의한다. 목표는 제품 repo를 public code/private authority
기준으로 관리하고, 새 repo와 release repo까지 같은 정책을 재사용하게 만드는
것이다.
...
- 제품 repo는 기본 public으로 운영한다. 코드 공개를 전제로 하되 merge/write 권한,
  secret 접근, production deploy 권한은 멤버와 승인된 automation으로 제한한다.
- private repo가 정말 필요하면 예외로 선언하고, GitHub 플랜 한계 때문에 적용할 수
...
| `public-product` | `hypeproof-studio`, `sediment`, 공개 전환 완료 후 `hypeprooflab` | public 유지, 외부 PR 허용, main은 PR/CI/review로만 유입, Actions read-only |
| `private-product` | private 제품 repo | 멤버만 접근, main PR 강제, 가능한 경우 branch protection 적용 |
| `content-vault` | 현재 `hypeprooflab`, 향후 private 콘텐츠 vault repo | 멤버만 접근, 콘텐츠 ingest secret 보호, provider Git 자동 배포 비활성화 |
...
Private repo는 예외 경로다. 접근 자체가 멤버에게만 허용되더라도 main 보호 원칙은 같다.
...
`hypeprooflab`은 public 전환 대상이지만 현재 desired state는 `private` +
`content-vault`다. Google OAuth client secret 노출 이력과 public-readiness PII
정리가 끝나기 전에는 public code/private authority 모델을 적용하지 않는다.
전환 조건:

- `hypeprooflab#96`: Google OAuth secret rotation/delete와 GitHub historical
  commit purge가 완료되고, offending commit이 GitHub API/direct SHA fetch로 더
  이상 접근되지 않는다.
- `hypeprooflab#100`: current tree의 PII/email sweep이 완료된다.
- fresh audit에서 위 두 blocker가 닫힌 상태와 current-tree secret/PII scan 통과가
  확인된다.

전환 PR은 그때 `policy/repos.yaml`의 `hypeprooflab`을 `visibility: public`,
`profile: public-product`로 바꾸고 이 예외들을 제거한다.
...
향후 private repo를 유지해야 하는 운영 선택지는 둘 중 하나다.
...
| `jayleekr/hypeprooflab` | private | `content-vault` | #96/#100 완료 전까지 private 유지, 완료 후 public-product 전환 |
