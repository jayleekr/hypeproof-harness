...
`hypeprooflab`처럼 personal private repo에 남아 있는 동안은 다음 항목도
unsupported drift로 추적한다.

- branch protection: GitHub Pro/Team 또는 Organization 필요
- `allow_forking`: GitHub API가 org-owned private repo mutation만 지원
- secret scanning: private repo `security_and_analysis` API가 현재 플랜에서 미노출

이 항목들은 `policy/repos.yaml`의 만료일 있는 exception으로 기록한다. exception은
드리프트를 숨기기 위한 장치가 아니라, 해결 책임자와 재검토 시점을 남기는
risk acceptance다.
