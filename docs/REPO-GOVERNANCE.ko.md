...
- 제품 repo는 기본 public으로 운영한다. 코드 공개를 전제로 하되 write 권한,
  deploy 권한, secret 접근은 멤버와 승인된 automation으로 제한한다.
...
| `content-vault` | 향후 private 콘텐츠 vault repo | 멤버만 접근, ingest secret 보호, vault 변경 workflow 제한 |
...
현재 제품 repo는 비용과 강제 정책 일관성을 위해 public을 기본값으로 둔다.
향후 정말 private repo가 필요하면 이 항목들은 `policy/repos.yaml`의 만료일 있는
exception으로 기록한다. exception은 드리프트를 숨기기 위한 장치가 아니라,
해결 책임자와 재검토 시점을 남기는 risk acceptance다.
...
| `jayleekr/hypeprooflab` | public | `public-product` | public repo 보호 정책으로 통일 |
