...
운영 정책 `pr_report_transport=issue`는 PR 저장소의 전용 검토 이슈에 PR 링크·고정 head와
영향 보고서를 게시한다. PR 번호 marker로 갱신하고 사람 본문은 보존한다. PR 댓글 쓰기
권한은 일반 이슈 쓰기와 다르다. 첫 main 실행 34253737163은 20개 검토 이슈를 발행했지만
