...
| **T-V15** | Preserve-extra overlay | `--preserve-extra` reports consumer-only files without failing a clean canonical comparison, never deletes them during apply, and still fails when a canonical file is changed. The default mode continues to reject extras. |
